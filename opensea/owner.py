...
base_url = "https://api.opensea.io/api/v1/assets?format=json&include_orders=true&limit=50&order_by=pk&order_direction=desc&owner={0}"
next_base_url = 'https://api.opensea.io/api/v1/assets?format=json&include_orders=true&limit=50&order_by=pk&order_direction=desc&owner={0}&cursor={1}'
...
sql = "select ir_owner_address from nft_owners where owner_state=1"
slugs = db.find(sql)
...
def run():
    for slug in slugs:
        ...
        while next:
            ...
            sql = "UPDATE `nft_owners` SET `owner_state` = 2 where ir_owner_address='{0}'".format(slug)
            db.update(sql)
