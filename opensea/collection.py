...
base_url = "https://api.opensea.io/api/v1/assets?format=json&include_orders=true&limit=50&order_by=pk&order_direction=desc&collection_slug={0}"
next_base_url = 'https://api.opensea.io/api/v1/assets?format=json&include_orders=true&limit=50&order_by=pk&order_direction=desc&collection_slug={0}&cursor={1}'
...
sql = "select distinct ir_collection_slug from nft_collections where collection_state=0"
slugs = db.find(sql)
...
def save_contract(item):
    ...
    """
    contract_dict = dict()
    contract_dict['ir_contract_address'] = item['ir_contract_address']
    contract_dict['contract_state'] = '0'
    db.add_smart('nft_contracts', contract_dict)
...
def run():
    for slug in slugs:
        ...
        while next:
            ...
            sql = "UPDATE `nft_collections` SET `collection_state` = 1 where ir_collection_slug='{0}'".format(slug)
            db.update(sql)
