...
base_url = "https://api.opensea.io/api/v1/assets?format=json&include_orders=true&limit=50&order_by=pk&order_direction=desc"
next_base_url = 'https://api.opensea.io/api/v1/assets?format=json&include_orders=true&limit=50&order_by=pk&order_direction=desc&cursor={0}'
...
@retry(stop_max_attempt_number=10)
def get_nft_next_text(next):
    """
    访问翻页接口链接获取NFT信息
    :param next:
    :return:
    """
    url = next_base_url.format(next)
    logger.info(url)
    resp = scraper.get(url, proxies=proxy)
    text = resp.text
    text = json.loads(text)
...
def run():
    next, text = get_nft_text()
    parse_nft(text)
    while next:
        next, text = get_nft_next_text(next)
        parse_nft(text)
